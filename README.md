...
### JWT Stateless 인증
- **Access Token**: 서버에 저장하지 않음. JWT 서명 검증만으로 인증.
- **Refresh Token**: DB에 저장하여 토큰 갱신 시 검증.
- **장점**: DB 조회 없이 인증 처리, 수평 확장 용이, 높은 성능.

자세한 내용은 [JWT Stateless 마이그레이션 문서](docs/JWT_STATELESS_MIGRATION.md) 참고.
